...
export const GET: APIRoute = async ({ request, params }) => {
    try {
        logger.dev('Services request received:', {
        ...
        const response = await fetch(`${backendURL}/service?userId=${userId}`, {
            method: 'GET',
            credentials: 'include',
            headers: {
        ...
        if (response.ok) {
            logger.dev('Services data retrieved successfully:', { userId })
        ...
        logger.error('Error en obtención de servicios:', error)
        return new Response(JSON.stringify({
